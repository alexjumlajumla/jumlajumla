...
class PaymentPayloadService extends CoreService
{
    ...
    public function prepareValidate($data): array
    {
        ...
        if ($payment->tag === Payment::TAG_PAY_PAL) {
            ...
            return ['status' => true];
        } else if ($payment->tag === Payment::TAG_SELCOM) {

            $validator = $this->selcom($data);
    ...
    public function selcom(array $data): \Illuminate\Contracts\Validation\Validator|\Illuminate\Validation\Validator
    {
        return Validator::make($data, [
            'payload.selcom_key'        => 'required|string',
            'payload.selcom_secret'        => 'required|string',
            'payload.selcom_vendor_id'         => 'required|string',
            'payload.currency'      => [
                'required',
                Rule::exists('currencies', 'title')
            ],

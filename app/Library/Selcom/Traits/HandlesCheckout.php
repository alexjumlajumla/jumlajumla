...
use App\Library\Selcom\Exceptions\InvalidDataException;
...
trait HandlesCheckout
{
    ...
    public function cardCheckoutUrl(array $data)
    {
        $this->validateCardCheckoutData($data);
        ...
        return $this->handleOrderResponseUrl($orderRequest, $data, $orderId, true);
    }

    private function handleOrderResponseUrl(Response $response, array $data, string $orderId, $cardPayment = false)
    {
    ...
    private function getMinimalOrderData(array $data, string $orderId): array
    {
        return [
            ...
            'webhook' => base64_encode($this->webhookUrl()),
